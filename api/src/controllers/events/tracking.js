...
const findAll = async (req, res) => {
  const trackings = await TrackingEvents.find({ appId: req.user.id });
  res.status(200).json(trackings);
};

const findOne = async (req, res) => {
  const tracking = await TrackingEvents.findOne({
    _id: req.params.id,
    appId: req.user._id,
  });

  if (!tracking)
    return res.status(404).json({ message: "MouseMovement not found" });
...
const create = async (req, res) => {
  if (
    !["click", "hover", "submit", "change", "focus", "blur"].includes(
      req.body.data.eventType
    )
  ) {
    return res
      .status(400)
      .json({ message: "EventType isn't compliant to available event types" });
  }
