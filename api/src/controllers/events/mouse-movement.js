...
const findAll = async (req, res) => {
  const mouseMovements = await MouseMovementEvents.find({ appId: req.user.id });

  res.status(200).json(mouseMovements);
};

const findOne = async (req, res) => {
  const mouseMovement = await MouseMovementEvents.findOne({
    _id: req.params.id,
    appId: req.user._id,
  });
...
const create = (req, res) => {
  ...
  mouseMovement
    .save()
    .then((mouseMovement) => res.status(201).json(mouseMovement));
};
